...
def makeCrowd(group):
    ...
    with group:
        ...
        crowdIDStr = 'crowdID'
        nuke.Layer(crowdIDStr, [crowdIDStr+'.id', crowdIDStr+'.sprite'])  # corrected line
...
def bakeTileColor(node):
    ...
    tc_hex = int('%02x%02x%02x%02x' % (int(tc_rgb[0]*255), int(tc_rgb[1]*255), int(tc_rgb[2]*255), 1), 16)
